...
from models import Player, PlayerCreate, Match, MatchCreate, ScoreUpdate
    ...
    try:
        ...
        try:
            ...
            if match.match_type == "2v2":
                if not all([match.team1_player1_id, match.team1_player2_id, 
                ...
                players = [match.team1_player1_id, match.team1_player2_id,
                         match.team2_player1_id, match.team2_player2_id]
            ...
            if match.match_type == "1v1":
                if match.team1_player2_id is not None or match.team2_player2_id is not None:
                    raise HTTPException(
                        status_code=422,
                        detail="1v1 matches should not have secondary players"
                    )
            ...
            if len(found_players) != len(set(player_ids)):
                raise HTTPException(
                    status_code=422,
                    detail="One or more players not found in database"
                )
            ...
            if match.match_date < datetime.now() and status == 'SCHEDULED':
                raise HTTPException(
                    status_code=422,
                    detail="Scheduled matches cannot be in the past"
                )
            ...
            cur.execute("""
                ...
                RETURNING *
            """, (
                match.round, match.match_type,
                match.team1_player1_id, match.team1_player2_id,
                match.team2_player1_id, match.team2_player2_id,
                match.match_date, scheduled_date,
            ...
            cur.execute("""
                WITH round1_stats AS (
                    SELECT 
                        ...
                        SUM(CASE 
                            WHEN (m.team1_player1_id = p.player_id AND COALESCE(m.team1_goals, 0) > COALESCE(m.team2_goals, 0)) OR
                                 (m.team2_player1_id = p.player_id AND COALESCE(m.team2_goals, 0) > COALESCE(m.team1_goals, 0)) THEN 1
                            ELSE 0
                        END) as wins,
                        SUM(CASE 
...
async def get_overview_stats():
    try:
        ...
        try:
            ...
            progress_percentage = (progress_stats['matches_played'] / progress_stats['total_expected_matches'] * 100) if progress_stats['total_expected_matches'] > 0 else 0
            ...
            cur.execute("""
				...
				streak_matches AS (
				    ...
				    WHERE 
				        (m.team1_goals > m.team2_goals AND (m.team1_player1_id = cs.player_id OR m.team1_player2_id = cs.player_id))
				        OR (m.team2_goals > m.team1_goals AND (m.team2_player1_id = cs.player_id OR m.team2_player2_id = cs.player_id))
				    ORDER BY m.match_date DESC
            ...
            return {
				...
				} if highest_scoring and highest_scoring['team1_goals'] is not None else None,
				"currentStreak": {
				    ...
				    "lastMatch": streak['last_match_date'].strftime("%Y-%m-%d") if streak and streak['last_match_date'] else None
				} if streak else None,
				"bestDefense": {
				...
				"cleanSheets": {
				    ...
				    ] if clean_sheets_data and clean_sheets_data['matches_detail'] else []
				} if clean_sheets_data else None
			}
        finally:
            cur.close()
            conn.close()
